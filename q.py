...
class Sqlite3DB(object):
    ...
    def __init__(self, show_sql=SHOW_SQL):
        ...
        self.type_names = {
            str: 'TEXT', int: 'INT', int : 'INT' , float: 'FLOAT', None: 'TEXT'}
        self.numeric_column_types = set([int, int, float])
    ...
    def store_db_to_disk_standard(self,sqlite_db_filename,table_names_mapping):
        ...
        for source_filename_str,tn in table_names_mapping.items():
    ...
    def generate_create_table(self, table_name, column_names, column_dict):
        ...
        column_name_to_db_type = dict(
            (n, self.type_names[t]) for n, t in column_dict.items())
...
class Sql(object):
    ...
    def __init__(self, sql):
        ...
        while idx < len(self.sql_parts):
            ...
            if part.upper() in ['FROM', 'JOIN']:
                ...
                if qtable_name not in list(self.qtable_name_positions.keys()):
    ...
    def set_effective_table_name(self, qtable_name, effective_table_name):
        ...
        if qtable_name in list(self.qtable_name_effective_table_names.keys()):
            raise Exception(
    ...
    def get_effective_sql(self,original_names=False):
        if len([x for x in self.qtable_name_effective_table_names if x is None]) != 0:
...
class TableColumnInferer(object):
    ...
    def determine_type_of_value_list(self, value_list):
        ...
        if len(set(type_list)) == 1:
            ...
            type_list_without_nulls = [x for x in type_list if x is not None]
    ...
    def validate_column_names(self, value_list):
        ...
        for v in value_list:
            ...
            if len([x for x in value_list if x == v]) > 1:
    ...
    def get_column_count_summary(self, column_count_list):
        ...
        return ", ".join(["%s rows with %s columns" % (v, k) for k, v in counts.items()])
    ...
    def infer_column_types(self):
        ...
        comparison = [x[0] == x[1] for x in zip(self.column_types, self.column_types2)]
    ...
    def get_column_dict(self):
        return dict(list(zip(self.column_names, self.column_types)))
...
class TableCreator(object):
    ...
    def _pre_populate(self,dialect):
        ...
        for filename in self.materialized_file_list:
            if filename in list(self.materialized_file_dict.keys()):
                continue
...
def determine_max_col_lengths(m,output_field_quoting_func,output_delimiter):
    ...
    for row_index in range(0, len(m)):
        for col_index in range(0, len(m[0])):
            new_len = len(str(output_field_quoting_func(output_delimiter,m[row_index][col_index])))
...
class QTableStructure(object):
    ...
    def __str__(self):
        return "QTableStructure<filenames_str=%s,materialized_file_count=%s,column_names=%s,column_types=%s>" % (
            self.filenames_str,len(list(self.materialized_files.keys())),self.column_names,self.column_types)
    __repr__ = __str__
...
class QTextAsData(object):
    ...
    def _load_data(self,filename,input_params=QInputParams(),stdin_file=None,stdin_filename='-',stop_after_analysis=False):
        ...
        if filename in list(self.table_creators.keys()) and filename != stdin_filename:
            return None
    ...
    def unload(self):
        ...
        for filename,table_creator in self.table_creators.items():
            try:
...
def quote_minimal_func(output_delimiter,v):
    if v is None:
        return v
    t = type(v)
    if t == str or t == str and ((output_delimiter in v) or ('"' in v)):
        ...
        try:
            ...
            for rownum, row in enumerate(data):
                ...
                for i, col in enumerate(row):
                    if formatting_dict is not None and str(i + 1) in list(formatting_dict.keys()) and not skip_formatting:
...
def run_standalone():
    ...
    def get_option_with_default(p, option_type, option, default):
        ...
        if option_type == 'boolean':
            ...
            return p.get('options', option).decode('unicode_escape')
        else:
    ...
    #             query_strs[idx] = query_strs[idx].options.query_encoding.strip()
    ...
    if options.input_quoting_mode not in list(QTextAsData.input_quoting_modes.keys()):
        print("Input quoting mode can only be one of %s. It cannot be set to '%s'" % (",".join(list(QTextAsData.input_quoting_modes.keys())),options.input_quoting_mode), file=sys.stderr)
        sys.exit(55)

    if options.output_quoting_mode not in list(QOutputPrinter.output_quoting_modes.keys()):
        print("Output quoting mode can only be one of %s. It cannot be set to '%s'" % (",".join(list(QOutputPrinter.output_quoting_modes.keys())),options.input_quoting_mode), file=sys.stderr)
        sys.exit(56)
